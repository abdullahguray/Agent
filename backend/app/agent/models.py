...
=====================
All available models via NVIDIA API. Each model has:
- id: model string for API calls
- name: display name
...
MODEL_REGISTRY = [
    # ─── REASONING MODELS (thinking/reasoning) ───
    {
    ...
    # ─── BALANCED MODELS (good all-round) ───
    {
...
def get_models_by_category(category: str = None) -> list:
    if category:
        return [m for m in MODEL_REGISTRY if m["category"] == category]
...
def get_model_params(model_id: str) -> dict:
    ...
    base = {
        "model": model_id,
        "temperature": model["params"].get("temperature", 0.3),
        "top_p": model["params"].get("top_p", 0.95),
        "max_tokens": model["params"].get("max_tokens", 1024)
    }
    extra = model["params"].get("extra_body", {})
    if extra:
        base["extra_body"] = extra
    for k in ["seed", "frequency_penalty", "presence_penalty", "repetition_penalty", "top_k", "chat_template_kwargs"]:
        if k in model["params"]:
            base[k] = model["params"][k]
    return base
